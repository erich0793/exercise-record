...
| `ocr` | `basePath` / `percentiles` / `psmModes` / `upscale` / `detect`（`percentiles` 候選門檻、`maxRows` 保留列數、`maxRegions` 逐值區域上限）/ `adaptive` / `speedBandKmh` / `mileToKm` | 辨識率不佳時可增減百分位組合；`maxRegions` 調小可加快、調大可提高讀到率 |
...
**執行結果：20 / 20 通過**（於 Node 22 與 Chromium 兩種環境執行，結果一致）
