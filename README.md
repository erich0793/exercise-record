...
不需框選。`detectValueGroups()` 以高百分位（`CONFIG.ocr.detect.percentile`，現為 98）二值化整張照片，用連通元件找出「像字元」的區塊，依垂直位置分列後取字元最多的一列（即跑步機的主要數值列），再依水平間距切成數值群組。每個群組送辨識後由 `classifyReadout()` 歸類為距離／時間／高度／卡路里。

實測樣本照片：跑步機畫面自動找到 **4 個數值群組**（0m、2.8km、17:12、206），耗時 6.3 秒；Nike Run Club 截圖經整列複核取得 `2.46KM00:15:3006:18M`，判定距離 2.46 km、時長 00:15:30 並由配速交叉驗證，**耗時 2.4 秒**。偵測框會畫在照片上供使用者核對，歸類錯誤時可從清單自行指派。
...
若只讀到配速與距離，亦可反推時長。英里制版面（`1.53MI`、`07:50MI`）自動換算為公里。

**雙極性偵測**：偵測同時嘗試亮字暗底（多數運動 App 與跑步機）與暗字亮底（整頁截圖），取字元較多者，避免在以白色 UI 為主的截圖上完全失效。
...
**執行結果：19 / 19 通過**（於 Node 22 與 Chromium 兩種環境執行，結果一致）
