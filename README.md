...
- 依類別動態顯示欄位：
  ...
  - **跑步機讀數**（跑步專屬）→ 可拍照或匯入跑步機螢幕照片，填入**距離 + 時間**後自動換算速度、配速、時長與 MET；時間分成**「分」與「秒」兩格**輸入（見 3.8）
...
- **MET 到底怎麼知道**（含熱身與緩和的估計方式）：一般人無法感知自己的 MET，本節說明四個實務層級（選現成項目 → HIIT 動作選單 → talk test 二分判定 → 心率輔助），並並列呈現 talk test 效力的正反文獻與其在 HIIT 上的限制
...
**時間分成「分」「秒」兩格輸入**：原本是單一欄位、要求填 `17:12` 這種格式，但手機在數字輸入欄叫出的鍵盤**沒有「:」可打**（`inputmode="numeric"` 只給數字，`decimal` 也只多一個小數點），使用者根本輸入不了。改成兩個純數字欄位後，手機預設鍵盤就足夠，也不必再處理格式錯誤——非數字根本輸入不進去。

- 只填「分」也可以，秒視為 0
- 秒數填超過 59 會自動進位（`16 分 90 秒` → 17.5 分）
- 儲存格式仍是 `mm:ss` 字串（`meta.timeText`），舊紀錄照常載入
- OCR 讀到的 `h:mm:ss`（如 `00:15:30`）套用時自動換成 15 分 30 秒

**照片處理**：照片僅存於本機 `localStorage`，不上傳；辨識（3.10）也在本機完成。存檔前以 canvas 縮圖至最長邊 `CONFIG.photo.maxDimPx`（1200 px）並轉為 JPEG（品質 0.72）；實測 2.9 MB 的手機照片壓縮後約 106 KB（縮減 96%）。若 `localStorage` 配額不足，程式會**捨棄照片後重試儲存**，確保運動紀錄本身不會遺失，並以提示告知使用者。
...
**強度選單**：使用者無法感知自己的 MET，故課表的四個區段（熱身／高強度／恢復／緩和）各提供一個動作選單，選了即自動帶入 MET。選項值由 `presetMet()` 解析，來源為既有活動清單（跳繩、波比、飛輪衝刺、快走、伸展⋯⋯）或 ACSM 跑步方程式即時計算（衝刺跑、快跑、慢跑熱身），不另行新增未查證的數值。手動修改 MET 時選單自動清空，避免顯示與數值不符。

**選項直接標示強度分級**（`light・不計入` / `moderate` / `vigorous・計 2 倍`），因為熱身與緩和的強度落差極大且結果違反直覺：

| 常見做法 | MET | 計入方式 |
|---|---|---|
| 慢跑熱身（約 8 km/hr） | 8.6 | **vigorous・計 2 倍** |
| 輕鬆騎車／飛輪 | 6.8 | **vigorous・計 2 倍** |
| 快走（6.4 / 5.6 km/hr） | 5.0 / 4.3 | moderate・計入 |
| 徒手熱身操 | 3.8 | moderate・計入 |
| 慢走、動態／靜態伸展 | 2.3–2.8 | light・**不計入 MEM** |
| 站著休息 | 1.8 | light・**不計入 MEM** |

熱身與緩和只要達 moderate 以上就完整計入 MEM（2018 PAG 已取消 10 分鐘最短時長限制）；伸展屬 light，對達標無貢獻但仍計入 MET-min 與熱量。
...
不需框選。`detectValueGroups()` 二值化整張照片，用連通元件找出「像字元」的區塊，依垂直位置分列，再依水平間距把每列切成數值群組。每個群組送辨識後由 `classifyReadout()` 歸類為距離／時間／高度／卡路里。

**門檻不預設，改為逐張挑選。** 早期版本固定用 98 百分位，只適合跑步機主機那種「大字亮、背景暗」的畫面。實測最佳門檻其實逐張不同：

| 照片 | 98 百分位下的成列字元數 | 最佳門檻 | 該門檻下字元數 |
|---|---|---|---|
| 跑步機主機 | 26 | 98 | 26 |
| Nike Run Club 截圖 | 23 | 95 | 41 |
| Garmin 主機（天花板反光） | 4 | 88 | 51 |

Garmin 主機的照片有**比字還亮的天花板反光**，98 百分位只框得到反光。因此改為掃過 `CONFIG.ocr.detect.percentiles`（98/95/92/88/84，明暗兩種極性各試一次），取「分列後仍成列的字元數」最多的一組。曾嘗試「在最高分某個容差內取最嚴門檻」以減少 OCR 次數，但那會讓 Garmin 少讀到距離——已改回單純取最高分，寧可多花偵測時間也不漏讀。

**版面可能是橫列，也可能是直式清單。** 跑步機與 Nike 把數值排成一列，Garmin 主機則是每個數值各佔一行。因此保留字元最多的前 `maxRows`（6）列一併辨識，而非只取單一列。

**兩階段掃描：先整列、再逐值。** 整列辨識便宜（只跑單行模式）且是真正解得出答案的來源；逐值辨識貴（每個區域要跑數個前處理變體）又多是雜訊。故先跑整列，**只有當距離與時長都直接讀到、且配速獨立驗證通過時**才跳過逐值階段。這道「配速印證」是必要的：實測 Garmin 主機曾在整列階段把 `7.7` 當成距離、配上 `22:19`，形式完全合法卻是讀錯了欄位，少了配速印證就會被當成答案定案。逐值階段另以 `maxRegions`（8）限制區域數，依列輪流取字元最多的群組，確保每列都留得下來。

三張參考照片的實測結果（Chromium 端對端，含引擎載入）：

| 照片 | 版面 | 判定結果 | 真值 | 耗時 |
|---|---|---|---|---|
| 跑步機主機 | 橫列、無配速 | 2.8 km、17:12（9.77 km/hr） | 相符 | 49 秒 |
| Nike Run Club | 橫列、有配速 | 2.46 km、00:15:30（9.52 km/hr） | 相符 | 2.4 秒 |
| Garmin 主機 | 直式清單、有配速 | 3.51 km、22:19（9.44 km/hr） | 相符 | 5.2 秒 |

跑步機那張最慢，因為機台不顯示配速，無法在整列階段取得印證，必須把逐值階段完整跑完。偵測框會畫在照片上供使用者核對，歸類錯誤時可從清單自行指派。

**整列複核與擷取式解析**

部分運動 App 的版面（實測 Nike Run Club）數值間距與字元間距幾乎相同——實測字元間距序列為 `26 9 27 9 37 7 29 10 26 9 34 8 29 9 27`，最大間距 37 px 與一般字距 26–30 px 無法區分，**任何以間距切分的規則都無法可靠分開**。且冒號因元件過小被濾除，留下的空隙反而造成 `00:15:30` 被誤切為 `00:15` + `30`，前者被解讀為 15 秒而非 15 分。

因此除了逐群組辨識，另對**整列聯集區域**再辨識一次，並由 `extractReadouts()` 從合併字串中逐一擷取：

```javascript
extractReadouts('2.46KM00:15:3006:18M')
// → [{kind:'distance', value:'2.46', unit:'km'},
//    {kind:'duration', value:'00:15:30'},
//    {kind:'timeOrPace', value:'06:18'}]
```

`h:mm:ss` 一律視為時長（配速不會用此格式）；`mm:ss` 後接 `KM`／`MI`／`MIN/KM` 者判為配速，無單位者標為 `timeOrPace` 待交叉驗證。

**配速交叉驗證**

`resolveReadouts()` 以「配速 = 時長 ÷ 距離」檢核候選組合（容差 10%），配速相符者得 +100 分，票數用於同分時區分真實讀數與雜訊。這解決了同一畫面同時出現時長與配速（兩者皆為 mm:ss）的歧義：

| 指派方式 | 隱含配速 | 是否與讀到的 06:18 相符 |
|---|---|---|
| 時長 15:30、距離 2.46 km | 6.30 min/km | **相符 ✓** |
| 時長 06:18、距離 2.46 km | 2.56 min/km | 不符 ✗ |

若只讀到配速與距離，亦可反推時長；只讀到時長與配速時，亦可反推距離（`距離 = 時長 ÷ 每公里配速`，並要求配速短於總時長）。英里制版面（`1.53MI`、`07:50MI`）自動換算為公里。

**辨識專用的速度合理帶**

手動輸入的 `speedPlausibility()`（45 km/hr）是「明顯打錯」的最後防線，對 OCR 排歧太寬鬆。`resolveReadouts()` 另以 `CONFIG.ocr.speedBandKmh`（**3～25 km/hr**）淘汰自洽但不可能的組合：

| 讀到的值 | 隱含組合 | 隱含速度 | 判定 |
|---|---|---|---|
| `22:19`、`2:19` | 距離 9.63 km ÷ 22:19 | 25.9 km/hr | **擋下**（超過 5 公里世界紀錄配速，人類無法維持 22 分鐘） |
| `22:19`、`6:23` | 距離 3.5 km ÷ 22:19 | 9.4 km/hr | 採用 |

上限取 25 km/hr：馬拉松世界紀錄約 20.9 km/hr、5 公里約 23 km/hr，25 已留有餘裕，但足以擋掉把別的欄位誤認成距離所湊出的組合。

**雙極性偵測**：偵測同時嘗試亮字暗底（多數運動 App 與跑步機）與暗字亮底（整頁截圖），每種極性各掃過全部候選百分位，避免在以白色 UI 為主的截圖上完全失效。

**局部對比重試（Sauvola）**：第一輪判定失敗時，改用 Sauvola 局部自適應二值化（`CONFIG.ocr.adaptive`，視窗 61、k = 0.15/0.25、放大 4 倍）重跑一次。全域門檻對「反光比字亮」的低對比主機無效，局部門檻才讀得出 Garmin 的 `22:19` 與 `6:23`。

**手動框選（備援）**

自動抓不到時使用。`refineRect()` 會把使用者的框選**自動收緊到字元邊界**（框太鬆會讓背景進入二值化統計，實測會使「2.8」被讀成「28」），並同時辨識原框與收緊框、合併投票，避免收緊裁過頭時失去正確結果。

**門檻取自影像自身，而非固定值**

固定絕對門檻只在特定照片有效。改以 `percentileThreshold()` 取照片自身的亮度百分位（`CONFIG.ocr.percentiles`），並含明暗兩種極性。退化條件下的實測比較：

| 條件 | 固定門檻 | 百分位門檻 |
|---|---|---|
| 原圖 | ✓ | ✓ |
| 變暗 / 變亮 / 低對比 / 模糊 / 縮小 | 多數失敗 | ✓ |
| **總成功率** | — | **12/14** |

**投票與交叉驗證**

`pickOcrCandidate()` 依「出現次數優先、信心度次之」投票，並以 `isValidOcrValue()` 做格式把關。無合格候選時回傳 `null`。

`reconcileReadouts()` 再以**物理合理性**交叉驗證：OCR 最常見的錯誤是漏掉小數點，而 28 公里在 17:12 內需要 97 km/hr，不可能成立。系統會在距離與時間的候選組合中，挑出唯一速度合理者，即使它的票數較低——實測正是如此（`28` 得 2 票、`2.8` 得 1 票，最終正確選出 `2.8`），並向使用者說明修正原因。

`speedPlausibility()` 為最後一道防線：換算速度超過 `CONFIG.running.maxPlausibleSpeedKmh`（45 km/hr，約人類短跑世界紀錄）時直接攔下不予計入，手動輸入亦適用。

**不得謊報信心**

辨識失敗時，錯的不只是讀數，還有「宣稱讀到了」這件事。實測一張 Garmin 主機照片（天花板燈、窗簾條紋與人影同時反射在面板上）出現兩個問題，都已修正：

| 問題 | 原本行為 | 現在行為 |
|---|---|---|
| 只讀到距離、沒讀到時間 | 顯示「自動抓到：距離 9.4 km」並填入欄位 | 明講「沒有同時讀出距離與時間，無法自動換算」，且**不填入欄位** |
| 分母用「讀出此型別的次數」 | 20 次辨識只有 2 次讀出，顯示成「2/2 次一致」 | 顯示「20 次辨識中只有 2 次讀出」，並判為證據不足 |

第一項尤其要緊：換算需要距離**與**時間，只有距離根本無法計算；而那個 9.4 其實是主機最後一列的「當量」，10 次辨識只出現 1 次。若照樣填入欄位，使用者補上時間按下套用，就把錯的距離存進紀錄了。

`readingConfidence(votes, attempts)` 是共用判準：票數需 ≥ `CONFIG.ocr.confidence.minVotes`（2），且佔實際辨識次數的比例需 ≥ `minAgreement`（0.25）。自動掃描與手動框選兩條路徑都套用同一套標準。

**這張照片最後仍讀不出來。** 手動框住時間欄後，20 次辨識中 2 次讀出 `27:44`（正確是 `27:14`）——條紋反光正好橫過數字。加上局部對比（Sauvola）重跑一輪也沒有改善。演算法對這種反光沒有辦法，因此改為據實告知並附上拍攝建議：讓數字占滿畫面、鏡頭正對螢幕避開反光、對焦後再拍。

**安全設計**

- **結果一律先顯示於可編輯欄位，按下「套用到表單」才進入紀錄**，不會靜默寫入
- 顯示「實際辨識 N 次中有幾次讀出此值」；**證據不足時以紅字警示並將欄位邊框標紅**，明講可靠度低、請核對照片
- **只讀到距離或只讀到時間時不宣稱成功，且不填入欄位**（見 3.10「不得謊報信心」）
- 同時列出其他候選值
- 實測本專案樣本照片：自動抓取路徑距離 `2.8km`、時間 `17:12` 皆正確；手動框選路徑距離初判為 `28`，經交叉驗證自動修正為 `2.8`
...
| `ocr` | `basePath` / `percentiles` / `psmModes` / `upscale` / `detect`（`percentiles` 候選門檻、`maxRows` 保留列數、`maxRegions` 逐值區域上限）/ `adaptive` / `speedBandKmh` / `confidence`（`minVotes` 2、`minAgreement` 0.25）/ `mileToKm` | 辨識率不佳時可增減百分位組合；`maxRegions` 調小可加快、調大可提高讀到率 |
...
**執行結果：21 / 21 通過**（於 Node 22 與 Chromium 兩種環境執行，結果一致）
...
| **TC-21** | 讀數證據強度（票數 + 實際辨識次數） | 6/10、3/10 通過；2/10 不通過（比例 0.2 < 0.25）；1/10 不通過（票數 < 2）；`pickOcrCandidate` 回傳 votes=2、total=2、attempts=10 | 6/10 **通過**；3/10 **通過**；2/10 **不通過（lowAgreement）**；1/10 **不通過（fewVotes）**；pick votes=**2**、total=**2**、attempts=**10** | ✅ PASS |
| **TC-20** | 直式清單版面與速度合理帶（Garmin 主機實測） | `22:19 6:23` → 由配速反推距離 3.5 km；`3.51KM 22:19 6:23` → 3.51 km、9.44 km/hr；`22:19 2:19`（25.9 km/hr）→ null | 反推 **3.5 km**；完整 **3.51 km、9.44 km/hr**；超速組合 **null（已擋下）** | ✅ PASS |
| **TC-19** | 合併字串擷取與配速交叉驗證（Nike Run Club 版面實測） | `2.46KM00:15:3006:18M` → 距離 2.46、時長 00:15:30、9.52 km/hr；英里制 1.53mi→2.46km；僅配速+距離可反推時長；純雜訊 → null | 擷取 **distance, duration, timeOrPace**；判定 距離 **2.46**、時長 **00:15:30**、**9.52 km/hr**；英里制 **2.46 km**；反推**成功**；雜訊 **null** | ✅ PASS |
...
| **TC-12** | 跑步機讀數換算（實際畫面 2.8 km / 17:12） | 時間 17.2 分、速度 9.77 km/hr、MET 10.30、Vigorous、MEM 34.4、MET-min 177.2；時間 mm:ss／h:mm:ss／純分鐘皆正確解析；「分」「秒」兩格輸入、秒數進位、只填分皆正確 | 時間 **17.2** 分、速度 **9.77** km/hr、MET **10.30**、**vigorous**、MEM **34.4**、MET-min **177.2**（70 kg 估計 **207 kcal**，跑步機自身顯示 **206 kcal**） | ✅ PASS |
...
**照片自動辨識（三種版面，完整走完「上傳 → 自動抓取 → 套用到表單」）**

| 照片 | 自動判定 | 套用後表單 | 真值比對 |
|---|---|---|---|
| 跑步機主機 | 2.8 km、17:12 | 速度 9.77、MET 10.3、MEM 34.4 | 相符 ✓ |
| Nike Run Club 截圖 | 2.46 km、00:15:30（配速 06:18 交叉驗證） | 速度 9.52、MET 10.07 | 相符 ✓ |
| Garmin 主機（反光、直式清單） | 3.51 km、22:19 | 速度 9.44、MET 9.99 | 相符 ✓ |
